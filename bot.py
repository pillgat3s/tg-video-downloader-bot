        ...
        with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if is_instagram_url(url) and any(k in err_lower for k in ("login required", "rate-limit", "not available", "cookies", "checkpoint")):
                    if COOKIES_FILE.exists():
                        await status_msg.edit_text(
                            "❌ Instagram rejected the download even though cookies are set — "
                            "they've probably expired or the session was logged out.\n"
                            "Re-export fresh cookies (log into instagram.com in your browser first) "
                            "and upload them again with /setigcookies."
                        )
                    else:
                        await status_msg.edit_text(
                            "❌ Instagram is blocking this download — login required.\n"
            ...
            if size > MAX_SIZE_BYTES:
                ...
                await status_msg.edit_text(
                    ...
                    "Telegram limits uploads to 50 MB.\n"
                    "Want me to compress it to fit? Quality is reduced only as "
                    "much as needed to get under the limit.",
...
async def handle_compress_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "no":
        await query.edit_message_text("Okay, skipping this download.")
        return
    ...
    await query.edit_message_text("⏳ Re-downloading the video…")
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            output_template = os.path.join(tmpdir, "video.%(ext)s")
            video_path, info = download_video(url, output_template)
            await query.edit_message_text(
                "🗜 Compressing to fit under 50 MB… this can take a few minutes."
            )
            out_path = compress_to_limit(video_path, tmpdir)
            size = os.path.getsize(out_path)
            if size > MAX_SIZE_BYTES:
                await query.edit_message_text(
                    f"❌ Still too large after compression ({size/1024/1024:.1f} MB). "
                    "This video can't fit under Telegram's limit."
                )
                return
            await query.edit_message_text("📤 Sending…")
            with open(out_path, "rb") as f:
        ...
        await query.edit_message_text("❌ Failed to compress the video.")
    except Exception:
...
HELP_TEXT = (
    ...
    "/gif — Reply to one of my videos or a video sticker to convert it to a GIF\n"
    "/sticker — Reply to one of my videos to convert it to a Telegram sticker\n"
    "/mp4 — Reply to a GIF or video sticker to convert it to a proper video\n"
...
async def handle_gif_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if reply and reply.sticker:
        ...
        if sticker.is_animated:
            await msg.reply_text("❌ Animated stickers (.tgs) can't be converted — only video stickers are supported.")
            return
        if not sticker.is_video:
            await msg.reply_text("❌ Only video stickers can be converted to GIF.")
            return
        if (sticker.file_size or 0) > 20 * 1024 * 1024:
            await msg.reply_text("❌ Sticker is too large to download.")
            return
        status_msg = await msg.reply_text("⏳ Converting sticker to GIF…")
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if size > 50 * 1024 * 1024:
                    await status_msg.edit_text("❌ GIF is too large. Try a shorter clip.")
                    return
                await status_msg.edit_text("📤 Sending…")
                with open(out_path, "rb") as f:
            ...
            await status_msg.edit_text("❌ Failed to convert sticker to GIF.")
        except Exception:
    ...
    if not reply or not reply.video:
        await msg.reply_text("Reply to one of my videos or a video sticker with /gif to convert it to a GIF.")
        return
    video = reply.video
...
async def handle_mp4_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not reply or (not reply.animation and not reply.sticker):
        await msg.reply_text("Reply to a GIF or video sticker with /mp4 to convert it to a proper video.")
        return
    ...
    if reply.sticker:
        ...
        if sticker.is_animated:
            ...
            return
        if not sticker.is_video:
            await msg.reply_text("❌ Only video stickers can be converted to MP4.")
            return
        status_msg = await msg.reply_text("⏳ Converting sticker to MP4…")
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                in_path  = os.path.join(tmpdir, "input.webm")
                ...
                size = os.path.getsize(out_path)
                if size > MAX_SIZE_BYTES:
                    await status_msg.edit_text(f"❌ Converted file is too large ({size/1024/1024:.1f} MB).")
                    return
                await status_msg.edit_text("📤 Sending…")
                with open(out_path, "rb") as f:
                    await msg.reply_video(
            ...
            await status_msg.edit_text("❌ Failed to convert sticker to MP4.")
        except Exception:
